#!/bin/bash
# =============================================================================
# NODE 2: Finetune BD3LM (bs=1, bs=8, bs=16) on UltraChat 200k (v3), then evaluate.
# =============================================================================
# v3: 20 epochs (~6780 total steps), save every 85 steps (~80 checkpoints),
# evaluate 20 evenly spaced (+ base + final).
# Eval uses DUEL exact likelihood (prob_margin, k=1) with --log_samples.
# Val NLL on held-out val split.
#
# Usage: bash scripts/finetune/run_ultrachat_v3_bd3lm.sh [2.8b]
# =============================================================================
...
NUM_TRAIN_EPOCHS=20
SAVE_STEPS=85                   # ~80 checkpoints across 20 epochs (total_steps ~ 6780)
SAVE_TOTAL_LIMIT=82             # keep all ~80 + final + buffer
NUM_CKPTS=20                    # eval 20 evenly spaced (+ base + final)
INCLUDE_BASE=1
...
# ── Step 1: Preprocess (shared, with val split — reuses v2 preprocessed data) ──
if [[ ! -d "${PREPROCESSED_DIR}/train" ]]; then
    echo "=== Preprocess UltraChat 200k (with val split) ==="
...
echo "============================================================"
echo "NODE 2: BD3LM (bs=1, bs=8, bs=16) | v3 | Size: ${SIZE}"
echo "  Data: ${PREPROCESSED_DIR}"
echo "  Epochs: ${NUM_TRAIN_EPOCHS} | Save every ${SAVE_STEPS} steps (~80 ckpts)"
...
# ── Step 3: Evaluate (DUEL with --log_samples) ───────────────────────
...
# format: diff_model|model_type|block_size|run_dir
declare -a RUN_SPECS=(
    "a2d_bd3lm|bd3lm|1|${BS1_DIR}"
    "a2d_bd3lm|bd3lm|8|${BS8_DIR}"
    "a2d_bd3lm|bd3lm|16|${BS16_DIR}"
)

echo "=== Eval BD3LM models (DUEL --log_samples + val NLL) ==="
...
echo ""
echo "============================================================"
echo "NODE 2 COMPLETE. Results:"
echo "  Training:  results/finetune/pythia-${SIZE}-bd3lm-bs{1,8,16}-${DATASET_TAG}/"
echo "  Eval:      results/finetune_eval_samples/bd3lm/..."
echo "============================================================"
