#!/bin/bash
# =============================================================================
# Finetune Pythia + Mamba (AR) on OpenMathInstruct-2, then evaluate.
# =============================================================================
# Trains AR models on math reasoning data, evaluates on:
#   - General cloze tasks (NLL curves): arc_easy, hellaswag, piqa, etc.
#   - Math-specific tasks: gsm8k_cot, minerva_math (accuracy)
#
# Sizing: set TRAIN_LIMIT to subsample (default: full dataset).
#   TRAIN_LIMIT=200000 bash scripts/finetune/run_math_ar.sh 2.8b
#
# Usage: bash scripts/finetune/run_math_ar.sh [2.8b]
# =============================================================================
...
MATH_TASKS="${MATH_TASKS:-gsm8k_cot}"
LAUNCH_DELAY=10

export PYTHONPATH="${PROJECT_ROOT}:${DLLM_ROOT}:${PYTHONPATH:-}"
export HF_DATASETS_TRUST_REMOTE_CODE=True

# ── Step 0: Ensure A2D Pythia exists (needed by preprocess for tokenizer) ──
...
echo "============================================================"
echo "MATH AR: Pythia + Mamba | Size: ${SIZE}"
...
# ── Step 5: Math-specific eval (gsm8k etc. -- final checkpoint only) ──
if [[ -n "${MATH_TASKS}" ]]; then
    echo ""
    echo "=== Math-specific eval (final checkpoints only) ==="
    PIDS=(); JOB_NAMES=(); GPU_IDX=0
    for spec in "${RUN_SPECS[@]}"; do
        IFS='|' read -r model_type run_dir base_model <<< "$spec"
        run_name="$(basename "$run_dir")"
        [[ ! -d "$run_dir" ]] && continue
        final_path="${run_dir}/checkpoint-final"
        [[ ! -d "$final_path" ]] && final_path="$run_dir"
        out_dir="${OUTPUT_ROOT}/${model_type}/${run_name}/checkpoint-final/reasoning_gen"
        [[ -d "$out_dir" ]] && { echo "[Skip] ${run_name}/math"; continue; }

        gpu="${GPU_ARRAY[$GPU_IDX]}"; GPU_IDX=$(( (GPU_IDX + 1) % NGPUS ))
        mkdir -p "$out_dir"
        echo "[Math Eval] ${model_type}/${run_name} on GPU ${gpu}"
        CUDA_VISIBLE_DEVICES="$gpu" lm_eval \
            --model hf \
            --model_args "pretrained=${final_path}" \
            --tasks "${MATH_TASKS}" \
            --num_fewshot 5 \
            --batch_size "${AR_BATCH_SIZE}" \
            --output_path "$out_dir" \
            > "$out_dir/eval.log" 2>&1 &
        PIDS+=("$!"); JOB_NAMES+=("${run_name}/math")
    done
    if (( ${#PIDS[@]} > 0 )); then echo "  Waiting for math evals..."
        for idx in "${!PIDS[@]}"; do set +e; wait "${PIDS[$idx]}"; rc=$?; set -e
            (( rc != 0 )) && echo "  Warning: ${JOB_NAMES[$idx]} exited $rc"; done; fi
fi
