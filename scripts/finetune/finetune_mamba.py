"""
Finetune Mamba on an instruction dataset using HuggingFace Trainer.

Usage (single node, 8 GPUs):
    torchrun --nproc_per_node=8 scripts/finetune/finetune_mamba.py \
        --model_name_or_path state-spaces/mamba-2.8b-hf \
        --output_dir results/finetune/mamba-2.8b-alpaca

    torchrun --nproc_per_node=8 scripts/finetune/finetune_mamba.py \
        --model_name_or_path state-spaces/mamba-1.4b-hf \
        --output_dir results/finetune/mamba-1.4b-alpaca
"""
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="results/finetune/mamba-2.8b-alpaca")
