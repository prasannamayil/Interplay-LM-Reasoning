#!/bin/bash
# =============================================================================
# MDLM Pythia-410M: A2D convert + 1 full epoch on 6.1B tokens + eval
# =============================================================================
# Pythia-410M: 24 layers, 1024 hidden, 16 heads (~393M params)
# Dataset: 19.3M examples, avg 317 tokens = 6.1B tokens total
# Batch:   32/GPU x 2 accum x 8 GPUs = 512 seqs/step (~162K tokens/step)
# Steps:   38,000 (~1 epoch)
# Saves:   every 2,000 steps (~19 checkpoints)
# Eval:    pass@128 on final checkpoint, ops 2,5,10,17
#
# Memory: ~410M params in bf16 ~ 0.8GB model + optimizer states ~4.8GB
#   ZeRO-2 shards optimizer across 8 GPUs. 32 seqs x 2048 max_len should
#   fit comfortably on 80GB A100s with gradient checkpointing enabled.
#
# Total time estimate: ~24-36h train + ~4h eval
# =============================================================================
...
OUTPUT_DIR="${PROJECT_ROOT}/results/gsm_infinity_ft_410m/pythia-410m-mdlm-1epoch"
...
# =========================================================================
# Step 2: Convert to A2D format (bidirectional attention + mask token)
# =========================================================================
...
# =========================================================================
# Step 3: Verify dataset compatibility
# =========================================================================
echo ""
...
# =========================================================================
# Step 4: Train MDLM 410M -- 1 full epoch
# =========================================================================
...
echo ""
echo "============================================================"
echo "Training MDLM Pythia-410M (1 epoch, 38K steps)"
echo "  Batch: 32/GPU x 2 accum x 8 GPUs = 512 seqs/step"
...
accelerate launch \
    ...
    --eval_strategy "no" \
    --report_to wandb \
    --run_name "pythia-410m-mdlm-1epoch" \
...
EVAL_DIR="${PROJECT_ROOT}/results/gsm_infinity_ft_410m/eval/pythia-410m-mdlm-1epoch/checkpoint-final_pass128"
mkdir -p "${EVAL_DIR}"
