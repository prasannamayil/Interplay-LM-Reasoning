#!/bin/bash
# =============================================================================
# BD3LM Pythia-410M: Conservative progressive schedule (1 → 32 → 64)
# =============================================================================
# Conservative variant: start at the AR limit (bs=1), then spend the bulk of
# training at the proven bs=32 sweet spot, then refine with bs=64.
#
# Rationale:
#   - bs=1 is exact AR: each token is its own block, so x_t[i] attends to all
#     prior x_0 positions — identical to causal attention. This maximally
#     leverages the pretrained Pythia AR weights before any diffusion kicks in.
#   - bs=32 is proven (38% pass@1 ID at 30K steps) — spend most compute here.
#   - bs=64 refinement: learns multi-step planning without forgetting bs=32.
#
# Schedule (1 epoch = 38K steps):
#   Phase 1:  0 -  4K steps  |  block_size=1   |  AR-exact warmup
#   Phase 2:  4K - 28K steps |  block_size=32  |  Main training (proven)
#   Phase 3: 28K - 38K steps |  block_size=64  |  Global planning refinement
#
# This is the "safe bet" — the model starts with full AR fidelity, then
# transitions to the proven block size, then refines with wider blocks.
#
...
RUN_NAME="pythia-410m-bd3lm-progressive-1-32-64"
...
# =========================================================================
# Phase definitions: "BLOCK_SIZE STEPS LR PHASE_NAME"
# =========================================================================
PHASES=(
    "1   4000  5e-5  phase1-bs1"
    "32  24000 5e-5  phase2-bs32"
    "64  10000 5e-5  phase3-bs64"
)

# =========================================================================
# Training: sequential phases, each loading previous checkpoint
# =========================================================================
PREV_MODEL="${A2D_DIR}"
...
#   phase2/ckpt-4000   (cum  8K, trained bs=32)
#   phase2/ckpt-8000   (cum 12K, trained bs=32)
#   phase2/ckpt-12000  (cum 16K, trained bs=32)
#   phase2/ckpt-18000  (cum 22K, trained bs=32)
#   phase2/final       (cum 28K, trained bs=32) *
#   phase3/ckpt-6000   (cum 34K, trained bs=64)
#   phase3/final       (cum 38K, trained bs=64) *
...
# "PHASE_DIR CKPT_NAME LABEL"
EVAL_CHECKPOINTS=(
    "phase1-bs1   checkpoint-final  cum04k-phase1end-bs1"
    "phase2-bs32  checkpoint-4000   cum08k-bs32"
    "phase2-bs32  checkpoint-8000   cum12k-bs32"
    "phase2-bs32  checkpoint-12000  cum16k-bs32"
    "phase2-bs32  checkpoint-18000  cum22k-bs32"
    "phase2-bs32  checkpoint-final  cum28k-phase2end-bs32"
    "phase3-bs64  checkpoint-6000   cum34k-bs64"
    "phase3-bs64  checkpoint-final  cum38k-phase3end-bs64"
)
...
# =========================================================================
echo ""
echo "========== PROGRESSIVE BD3LM (CONSERVATIVE) SUMMARY =========="
python3 -c "
import json, os
