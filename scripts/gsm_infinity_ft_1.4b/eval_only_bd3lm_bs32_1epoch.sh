#!/bin/bash
# =============================================================================
# BD3LM Pythia-1.4B: EVAL ONLY — finish the remaining ckpt-2500 eval
# =============================================================================
# 7/8 checkpoints are fully evaluated. ckpt-2500 is missing ops 19+20.
# This script runs ONLY the eval (no training), targeting ONLY incomplete
# checkpoints (those without metrics.jsonl). The per-op resume in
# eval_pass128.py skips ops whose details_op{N}.json already exists.
#
# Safe to run repeatedly — completed checkpoints are skipped entirely.
# =============================================================================
...
echo "============================================================"
echo "BD3LM-1.4B EVAL ONLY (random/256, ops 2-20, pass@128)"
...
for CKPT in "${CHECKPOINTS[@]}"; do
    ...
    if [[ -f "${OUT}/metrics.jsonl" ]]; then
        echo "[skip] Already fully evaluated: checkpoint-${CKPT}"
        continue
    fi

    done_ops=$(ls "${OUT}"/details_op*.json 2>/dev/null | wc -l)
...
if [[ ${#PIDS[@]} -eq 0 ]]; then
    ...
    echo ""
    echo "Waiting for ${#PIDS[@]} eval job(s)..."
    for pid in "${PIDS[@]}"; do
        wait "$pid"
