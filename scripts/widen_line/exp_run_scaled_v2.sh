...
case "${ARCH}" in
  ...
  mamba2)      # mamba2's conv input is a non-contiguous slice whose seq-stride = d_in_proj
               # (= 2*hidden + 2*ngroups*state + n_heads); the causal_conv1d kernel needs it
               # multiple of 8, which reduces to n_heads % 8 == 0. NH is mult-of-8 for s/l/xl
               # (8/16/24) but NOT for xs(4)/m(12) -> force mamba_n_heads to a multiple of 8.
               # For s/l/xl this equals NH (identical to before -> resume-safe).
               MH=${NH}; [ $((NH % 8)) -ne 0 ] && MH=8
               AOVR="model.arch_type=mamba2 model.n_layers=${NL} model.mamba_n_heads=${MH}"; GENFAITHFUL=0 ;;
