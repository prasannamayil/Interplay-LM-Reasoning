...
SUB="$PR/scripts/widen_line/condor/exp_gsm_hard.sub"
...
is_done(){ local f="$EXP/eval/$1_$2/ckpt-12000/metrics.jsonl"; [ -s "$f" ] && grep -q "pass@1" "$f"; }
newest_age(){ local now newest=0 t k="$1_$2"; now=$(date +%s)
  for f in "$EXP/$k/train.log" $(ls -1 "$EXP/eval/$k"/ckpt-*/metrics.jsonl 2>/dev/null) $(ls -1t "$LOGDIR/hard_${k}_"*.out 2>/dev/null|head -1); do
...
python "$PR/analyze/widen_line_gsm_hard.py" --no_plot 2>&1 | tail -25
