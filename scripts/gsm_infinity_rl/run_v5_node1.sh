#!/bin/bash
# =============================================================================
# V5 DPG experiments -- node 1 of 2
# Runs: GRPO baseline + DPG, DR-GRPO baseline + DPG(eta=0.5)
#
# 5 runs total: train sequentially, eval final checkpoint of each with
# process-verified pass@128 reward.
# =============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --eval-only|--skip-train) SKIP_TRAIN=true; shift ;;
...
RUNS=(
    "grpo_edge_v5"
    "grpo_dpg_eta05_edge_v5"
    "grpo_dpg_eta2_edge_v5"
    "dr_grpo_edge_v5"
    "dr_grpo_dpg_eta05_edge_v5"
)

RESULTS_BASE="results/gsm_infinity_rl_v5"
TOTAL=${#RUNS[@]}
TOTAL_START=$(date +%s)

echo "================================================================================"
echo "V5 DPG experiments -- node 1 (${TOTAL} runs)"
echo "================================================================================"
echo "Runs: ${RUNS[*]}"
