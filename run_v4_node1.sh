#!/bin/bash
# Node 1: Base model eval + grpo_id_v4 + grpo_mixed_v4 (train + process eval each)
set -e
...
# ── Step 1: Base model process-verified eval ──
EVAL_DIR="$PROJECT_ROOT/results/gsm_infinity_rl_v4/base_model_eval_pass128"
if [ -f "${EVAL_DIR}/metrics.jsonl" ]; then
    echo "Base model already evaluated, skipping."
else
    echo "================================================================"
    echo "[Node 1] Base Model process-verified pass@128"
    echo "================================================================"
    mkdir -p "$EVAL_DIR"
    python3 -m verl.trainer.main_ppo \
        --config-path "$CONFIG_DIR" \
        --config-name "grpo_edge_v4" \
        ...
        trainer.experiment_name="base_model_v4_eval" \
...
run_and_eval "grpo_id_v4" "grpo_id_v4"
run_and_eval "grpo_mixed_v4" "grpo_mixed_v4"

TOTAL_END=$(date +%s)
TOTAL_DURATION=$(( TOTAL_END - TOTAL_START ))
HOURS=$(( TOTAL_DURATION / 3600 ))
MINS=$(( (TOTAL_DURATION % 3600) / 60 ))
echo "Node 1 Complete! Total wall time: ${HOURS}h ${MINS}m"
